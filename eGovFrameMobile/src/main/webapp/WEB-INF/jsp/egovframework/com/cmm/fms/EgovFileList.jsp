...
<script type="text/javascript">
	...
	function fn_egov_downFile(atchFileId, fileSn){
		window.open("<c:url value='/cmm/fms/FileDown.mdo?atchFileId="+atchFileId+"&fileSn="+fileSn+"'/>");
	}
	...
	function fn_egov_deleteFile(atchFileId, fileSn) {
		...
		form.action = "<c:url value='/cmm/fms/deleteFileInfs.mdo'/>";
		form.submit();
	}
			...
			<c:choose>
				<c:when test="${updateFlag=='Y'}">
					...
					<img src="<c:url value='/images/egovframework/com/cmm/fms/icon/bu5_close.gif'/>" 
						width="19" height="18" onClick="fn_egov_deleteFile('<c:out value="${atchFileId}"/>','<c:out value="${fileVO.fileSn}"/>');" alt="파일삭제">
				</c:when>
				<c:otherwise>
