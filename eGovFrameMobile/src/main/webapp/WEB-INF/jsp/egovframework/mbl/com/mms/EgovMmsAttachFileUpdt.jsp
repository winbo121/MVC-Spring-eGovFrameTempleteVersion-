<% 
/**
 * @Class Name : EgovMmsAttachFileUpdt.jsp
 * @Description : 첨부파일 수정
...
<script type="text/javascript">
	...
	function fn_egov_updt_mmsAttachFile() {	
		...
		if (confirm("<spring:message code="common.update.msg"/>"))    {  
		    document.atchFile.action = "<c:url value='/mbl/com/mms/updateMmsAttachFile.mdo'/>";
		    document.atchFile.submit();
		}
	}

	function fn_egov_delete_mmsAttachFile() {	
		document.atchFile.atchFileDelFlag.value = "Y";
		document.atchFile.action = "<c:url value='/mbl/com/mms/goMmsAttachFileUpdt.mdo'/>";
		document.atchFile.submit();
	}
...
<noscript class="noScriptTitle">자바스크립트를 지원하지 않는 브라우저에서는 일부 기능을 사용하실 수 없습니다.</noscript>
	...
	<div id="content" class="contents2">
		<form modelAttribute="atchFile" name="atchFile" method="post" action="" enctype="multipart/form-data">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="table-register" summary="MMS 첨부파일 정보를 등록합니다.">
		  <tr> 
		    ...
		    <td width="80%" nowrap>
		    	<input type="text" name="mmsAtchFileSj" style="width:50%;height:100%;" value="<c:out value="${attachFile.mmsAtchFileSj}"/>"/>
		    </td>
		  </tr>
		  <tr> 
		    <th width="20%" height="23" nowrap >첨부파일
		    	<img src="<c:url value='/images/egovframework/mbl/mcomd/icon/required.gif'/>" width="15" height="15" alt="필수입력사항표시">
		    </th>
		    <td width="80%" nowrap>
		    <c:if test="${attachFile.atchFileId != ''}">
			   	<input type="text" name="atchFileNm" style="width:50%;height:100%;" value="<c:out value="${attachFile.atchFileNm}"/>" readonly/>
			   	<input type="button" name="btnDelete" value="삭제" onClick="javascript:fn_egov_delete_mmsAttachFile();">
		    </c:if>
		    <c:if test="${attachFile.atchFileId == ''}">
		...
		<table border="0" cellspacing="0" cellpadding="0" align="center">
			<tr> 
				...
				<td background="<c:url value='/images/egovframework/mbl/mcomd/button/bu2_bg.gif'/>" class="text_left" nowrap>
					<a href="JavaScript:fn_egov_updt_mmsAttachFile('<c:out value="${searchVO.sn}"/>');">수정</a>
				</td>
				<td>
				...
				<td background="<c:url value='/images/egovframework/mbl/mcomd/button/bu2_bg.gif'/>" class="text_left" nowrap>
					<a href="javascript:fn_egov_select_mmsAttachFileList();">목록</a>
					<input name="sn" type="hidden" value="<c:out value="${searchVO.sn}"/>">
					<input name="atchFileId" type="hidden" value="<c:out value="${attachFile.atchFileId}"/>">
					<input name="atchFileDelFlag" type="hidden" value=""> 
