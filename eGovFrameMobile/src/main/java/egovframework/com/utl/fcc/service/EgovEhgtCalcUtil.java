...
public class EgovEhgtCalcUtil {
  ...
  //2017.02.16 김준호 시큐어코딩(ES)-부적절한 예외 처리[CWE-253, CWE-440, CWE-754]
     ...
     * @see
     */
   public	void	readHtmlParsing(String str) throws Exception {
    	...
    	try
   	 	{
   		    ...
   		    //e.printStackTrace();
   	 		throw new EgovBizException("[EgovEhgtCalcUtil] : connection close fail");
		} finally {
			...
			if (reader != null) { // 2011.10.21 보안점검 후속조치
		        ...
		        //2017.02.17 김준호 시큐어코딩(ES)-부적절한 예외 처리[CWE-253, CWE-440, CWE-754]
					...
					LOGGER.error("["+ignore.getClass()+"] Try/Catch...IOException : " + ignore.getMessage());
		        //2017.02.17 김준호 시큐어코딩(ES)-부적절한 예외 처리[CWE-253, CWE-440, CWE-754]
				}
			}
