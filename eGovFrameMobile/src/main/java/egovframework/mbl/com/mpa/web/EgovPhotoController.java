...
public class EgovPhotoController {
    ...
    public ModelAndView selectMobilePhotoList(ModelMap model,
        ...
        List<EgovMap> PhotoList = (List<EgovMap>)photoService.selectPhotoList(searchVO);

        for (EgovMap photo : PhotoList) {
			String sessionId = request.getSession().getId();
			String toEncrypt = sessionId + "|" + photo.get("atchFileId");
			photo.setValue(3, Base64.getEncoder().encodeToString(
